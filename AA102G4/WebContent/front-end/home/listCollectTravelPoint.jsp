				...
				<div class="row">
					...
					<c:forEach var="travelVO" items="${travelSvc.all}">
						<c:if test="${travelVO.tra_del!=1}">
							<c:if test="${reportcollectSvc.getOneColTravelBymem_no(memberVO.mem_no,travelVO.tra_no).rc_col_status==0}">
									...
									<div class="box">
										...
										<div>
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/travel/travel.do" >
							                    <button type="submit" class="btn btn-primary">取消收藏</button>
							                    <input type="hidden" name="action" value="cancelCollectFromHome">
							                    <input type="hidden" name="rc_no" value="${reportcollectSvc.checkTravelCollect(travelVO.getTra_no(),memberVO.getMem_no()).rc_no}">
