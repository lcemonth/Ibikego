				...
				<div class="row">
					...
					<div>
						<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/member/member.do">
					        <input type="text" name="mem_name" placeholder="輸入姓名">
					...
					<table class="table table-hover">
	   					<thead>
							<tr>
								<th>大頭貼</th>
								<th width=80>姓名(暱稱)</th>
								<th width=80>連絡電話</th>
							</tr>
						</thead>
						<tbody>
							<c:if test="${empty searchMember}">
								<c:forEach var="noSearch" items="${list}">
									<c:if test="${(noSearch.mem_no!=memberVO.mem_no)&&(noSearch.mem_del!=1)}">
										<c:if test="${relationshipSvc.checkBlacklist(memberVO.mem_no,noSearch.mem_no)!=1}">
											<tr align='center' valign='middle'>
												<td><img src="<%=request.getContextPath()%>/member/memberPhoto.do?mem_no=${noSearch.mem_no}" height=80 width=80/></td>
												<td>${noSearch.mem_name}</td>
												<td>${noSearch.mem_phone}</td>
												<td>
													<c:if test="${relationshipSvc.checkFollow(memberVO.mem_no,noSearch.mem_no)==null}">
														<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/relationship.do">
														     ...
														     <input type="hidden" name="rel_mem_no" value="${noSearch.mem_no}">
												...
												<td>
													<c:if test="${relationshipSvc.checkBlacklist(memberVO.mem_no,noSearch.mem_no)==null}">
														<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/relationship.do">
							...
							<c:if test="${not empty searchMember}">
								<c:forEach var="search" items="${searchMember}">
									<c:if test="${(search.mem_no!=memberVO.mem_no)&&(search.mem_del!=1)}">
										<tr align='center' valign='middle'>
											<td><img src="<%=request.getContextPath()%>/member/memberPhoto.do?mem_no=${search.mem_no}" height=80 width=80/></td>
											<td>${search.mem_name} (${search.mem_nickname})</td>
											<td>${search.mem_phone}</td>
											<td>
												<c:if test="${relationshipSvc.checkFollow(memberVO.mem_no,search.mem_no)==null}">
													<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/relationship.do">
													     <input type="submit" class="btn btn-primary" value="追蹤">
													     <input type="hidden" name="mem_no_main" value="${memberVO.mem_no}">
													     <input type="hidden" name="rel_mem_no" value="${search.mem_no}">
													     <input type="hidden" name="action"	value="follow">
													</FORM>
												</c:if>
												<c:if test="${relationshipSvc.checkFollow(memberVO.mem_no,search.mem_no)!=null}">
											...
											<td>
												<c:if test="${relationshipSvc.checkBlacklist(memberVO.mem_no,search.mem_no)==null}">
													<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/relationship.do">
													     <input type="submit" class="btn btn-primary" value="封鎖">
													     <input type="hidden" name="mem_no_main" value="${memberVO.mem_no}">
													     <input type="hidden" name="rel_mem_no" value="${search.mem_no}">
