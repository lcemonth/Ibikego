...
List<BlogVO> listMyBlog = blogSvc.getMyBlogs(memberVO.getMem_no());
pageContext.setAttribute("listMyBlog",listMyBlog);
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="row">
					...
					<c:forEach var="blogVO" items="${listMyBlog}">
						<c:if test="${blogVO.blog_del!=1}">
							<a href="<%=request.getContextPath()%>/blog/blog.do?action=getOne_For_UserDisplay&blog_no=${blogVO.blog_no}">
								<div class="box">
									<img src="<%=request.getContextPath()%>/blogImage.do?blog_no=${blogVO.blog_no}" width="1024" height="628"/>
									<h3 id="title">${blogVO.blog_title}</h3>
									<p class="p1">${blogVO.blog_content}</p>
									<a href="<%=request.getContextPath()%>/blog/blog.do?action=getOne_For_UserUpdate&blog_no=${blogVO.blog_no}">
