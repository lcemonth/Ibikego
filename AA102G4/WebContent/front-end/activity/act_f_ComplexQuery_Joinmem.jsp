...
<jsp:useBean id="strokeSvc" scope="page"
	class="com.stroke.model.StrokeService" />	
<%
...
<ul>
  <li> 所有揪團活動<a href="<%=request.getContextPath()%>/activity/activity.do?action=listJoinAct_ByCompositeQuery&mem_no=<%=mem_no%>">
  ...
  <li>   
    <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/activity/activity.do" name="form1">
       			...
       			<c:forEach var="actVO" items="${list}" > 
		         	<c:forEach var="strokeVO" items="${strokeSvc.all}" > 
			         	<c:if test="${(actVO.stroke_no==strokeVO.stroke_no)}">
			          		<option value="${actVO.stroke_no}"  >${strokeVO.stroke_name}
			          	</c:if>	
		         	</c:forEach>  
	         	</c:forEach>   
       ...
       <br>
        ...
        <input type="hidden" name="action" value="listJoinAct_ByCompositeQuery">
     </FORM>
  </li>
</ul>
