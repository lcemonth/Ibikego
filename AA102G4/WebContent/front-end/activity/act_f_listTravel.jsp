...
<%  
	List<TravelVO> list = (List<TravelVO>) request.getAttribute("list");
	if(list==null){
					...
					$("#tra_no").val(idAry[1]);
					 if(idAry[0]=="bt2"){
						...
						function toC(){
							...
							$("#form1").attr("action","<%=request.getContextPath()%>/reportcollect/reportcollect.do");
							$("#form1_action").val("collectOneTravel");
							$("#tra_no").val(idAry[1]);
							$("#form1").submit();
					}
							...
							<c:forEach var="traVO" items="${list}" >
								<div class="col-xs-12 col-sm-3">
									<div class="thumbnail aaa">
											<div class="caption">
												<h3 style=color:#0066CC><b>${traVO.tra_name}</b></h3>
												<p class="txtlim">
													${traVO.tra_content}
												</p>
													<c:if test="${rcSvc.getOneRepTravelBymem_no(mem_no,traVO.tra_no)==null}">   
														<input type="button" id="bt2_${traVO.tra_no}" value="檢舉" class="btn btn-primary">
													</c:if>
													<c:if test="${rcSvc.getOneRepTravelBymem_no(mem_no,traVO.tra_no)!=null}">   
														<input type="button" id="bt2_${traVO.tra_no}" value="已檢舉" class="btn btn-default" disabled='true'>
													</c:if>
													<input type="button" id="bt3_${traVO.tra_no}" value="收藏" class="btn btn-primary">	  						
											</div>
									</div>
								</div>
							</c:forEach>
								...
								<FORM METHOD="post" id="form1" ACTION="">
								     ...
								     <input type="hidden" id="tra_no" name="tra_no" value="">
