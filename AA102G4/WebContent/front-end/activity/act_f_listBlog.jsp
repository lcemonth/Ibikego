...
/* 			white-space:nowrap; */
		...
		.box:hover{
			box-shadow: 0px 0px 2px 4px #64A600;
		}
		.aaa{
		cursor:pointer;
		}
					...
					$("#blog_no").val(idAry[1]);
					 if(idAry[0]=="bt2"){
						...
						function toC(){
							...
							$("#form1").attr("action","<%=request.getContextPath()%>/reportcollect/reportcollect.do");
							$("#form1_action").val("collectOneBlog");
							$("#blog_no").val(idAry[1]);
							$("#form1").submit();
					}
							...
							<c:forEach var="blogVO" items="${list}" >
								<div class="col-xs-12 col-sm-3">
									<div class="thumbnail aaa">
											<div class="caption">
												<h3 style=color:#0066CC><b>${blogVO.blog_title}</b></h3>
												<p class="txtlim">
													${blogVO.blog_content}
												</p>
													<c:if test="${rcSvc.getOneRepBlogBymem_no(mem_no,blogVO.blog_no)==null}">   
														<input type="button" id="bt2_${blogVO.blog_no}" value="檢舉" class="btn btn-primary">
													</c:if>
													<c:if test="${rcSvc.getOneRepBlogBymem_no(mem_no,blogVO.blog_no)!=null}">   
														<input type="button" id="bt2_${blogVO.blog_no}" value="已檢舉" class="btn btn-default" disabled='true'>
													</c:if>
													<c:if test="${rcSvc.getOneColBlogBymem_no(mem_no,blogVO.blog_no)==null}">  
														<input type="button" id="bt3_${blogVO.blog_no}" value="收藏" class="btn btn-primary">
													</c:if>
													<c:if test="${rcSvc.getOneColBlogBymem_no(mem_no,blogVO.blog_no)!=null}">  
														<input type="button" id="bt3_${blogVO.blog_no}" value="已收藏" class="btn btn-default" disabled='true'>
													</c:if>		  						
											</div>
									</div>
								</div>
