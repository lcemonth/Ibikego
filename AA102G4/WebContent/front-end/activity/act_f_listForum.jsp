					...
					$("#forum_no").val(idAry[1]);
					 if(idAry[0]=="bt2"){
						...
						function toC(){
							...
							$("#form1").attr("action","<%=request.getContextPath()%>/reportcollect/reportcollect.do");
							$("#form1_action").val("collectOneForum");
							$("#forum_no").val(idAry[1]);
							$("#form1").submit();
					}
							...
							<c:forEach var="forumVO" items="${list}" >
								<div class="col-xs-12 col-sm-3">
									<div class="thumbnail aaa">
											<div class="caption">
												<h3 style=color:#0066CC><b>${forumVO.forum_title}</b></h3>
												<p class="txtlim">
													${forumVO.forum_content}
												</p>
													<c:if test="${rcSvc.getOneRepForumBymem_no(mem_no,forumVO.forum_no)==null}">   
														<input type="button" id="bt2_${forumVO.forum_no}" value="檢舉" class="btn btn-primary">
													</c:if>
													<c:if test="${rcSvc.getOneRepForumBymem_no(mem_no,forumVO.forum_no)!=null}">   
														<input type="button" id="bt2_${forumVO.forum_no}" value="已檢舉" class="btn btn-default" disabled='true'>
													</c:if>
													<input type="button" id="bt3_${forumVO.forum_no}" value="收藏" class="btn btn-primary">	  						
											</div>
									</div>
								</div>
							</c:forEach>
								...
								<FORM METHOD="post" id="form1" ACTION="">
								     ...
								     <input type="hidden" id="forum_no" name="forum_no" value="">
